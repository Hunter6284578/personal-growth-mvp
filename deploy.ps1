# Deploy script for Windows PowerShell
# Usage:
#   .\deploy.ps1
#   .\deploy.ps1 -ServerIP 118.31.169.161 -SSHUser root -SSHPort 22

param(
    [string]$ServerIP = "118.31.169.161",
    [string]$SSHUser = "root",
    [int]$SSHPort = 22,
    [string]$ProjectDir = "/var/www/personal-growth-mvp",
    [string]$AppName = "personal-growth-mvp"
)

$ErrorActionPreference = "Stop"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Deploy personal-growth-mvp to ECS" -ForegroundColor Cyan
Write-Host "Server: $ServerIP" -ForegroundColor Cyan
Write-Host "Project dir: $ProjectDir" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

foreach ($command in @("git", "ssh", "scp")) {
    if (-not (Get-Command $command -ErrorAction SilentlyContinue)) {
        throw "Command not found: $command. Please install Git for Windows and OpenSSH."
    }
}

$ScriptDir = $PSScriptRoot
Set-Location $ScriptDir

$isGitRepo = git rev-parse --is-inside-work-tree
if ($isGitRepo.Trim() -ne "true") {
    throw "Current directory is not a Git repository."
}

$TempZip = Join-Path $env:TEMP "personal-growth-mvp.zip"
if (Test-Path $TempZip) {
    Remove-Item -LiteralPath $TempZip -Force
}

Write-Host "[1/5] Archive current commit..." -ForegroundColor Green
git archive --format=zip --output $TempZip HEAD

Write-Host "[2/5] Upload archive..." -ForegroundColor Green
$Target = "$SSHUser@$ServerIP"
$SshOptions = @("-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new")
scp -P $SSHPort @SshOptions $TempZip "${Target}:/tmp/personal-growth-mvp.zip"
Remove-Item -LiteralPath $TempZip -Force

Write-Host "[3/5] Install dependencies and build..." -ForegroundColor Green
$RemoteScript = @"
set -e

PROJECT_DIR="$ProjectDir"
APP_NAME="$AppName"
RELEASE_DIR="/tmp/`$APP_NAME-release-`$(date +%Y%m%d%H%M%S)"
BACKUP_DIR="/tmp/`$APP_NAME-env-backup"
OLD_DIR="`$PROJECT_DIR.backup-`$(date +%Y%m%d%H%M%S)"

rm -rf "`$RELEASE_DIR" "`$BACKUP_DIR"
mkdir -p "`$RELEASE_DIR" "`$BACKUP_DIR"

if [ -d "`$PROJECT_DIR/logs" ]; then
  cp -a "`$PROJECT_DIR/logs" "`$RELEASE_DIR/logs"
else
  mkdir -p "`$RELEASE_DIR/logs"
fi

for env_file in .env .env.local .env.production; do
  if [ -f "`$PROJECT_DIR/`$env_file" ]; then
    cp "`$PROJECT_DIR/`$env_file" "`$BACKUP_DIR/`$env_file"
  fi
done

unzip -q /tmp/personal-growth-mvp.zip -d "`$RELEASE_DIR"
rm -f /tmp/personal-growth-mvp.zip

for env_file in .env .env.local .env.production; do
  if [ -f "`$BACKUP_DIR/`$env_file" ]; then
    cp "`$BACKUP_DIR/`$env_file" "`$RELEASE_DIR/`$env_file"
  fi
done

cd "`$RELEASE_DIR"
npm ci
npm run build
npm prune --omit=dev

mkdir -p "`$(dirname "`$PROJECT_DIR")"
if [ -d "`$PROJECT_DIR" ]; then
  mv "`$PROJECT_DIR" "`$OLD_DIR"
fi
mv "`$RELEASE_DIR" "`$PROJECT_DIR"
"@

$RemoteScript | ssh -p $SSHPort @SshOptions $Target "bash -s"

Write-Host "[4/5] Restart PM2..." -ForegroundColor Green
ssh -p $SSHPort @SshOptions $Target "cd '$ProjectDir' && pm2 delete '$AppName' 2>/dev/null || true && pm2 start ecosystem.config.js --env production && pm2 save"

Write-Host "[5/5] Verify service..." -ForegroundColor Green
ssh -p $SSHPort @SshOptions $Target "pm2 list && curl -s -o /dev/null -w 'localhost:3000 HTTP %{http_code}\n' http://127.0.0.1:3000/"

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Deploy finished." -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Useful commands:" -ForegroundColor Yellow
Write-Host "  pm2 logs $AppName" -ForegroundColor Gray
Write-Host "  pm2 restart $AppName" -ForegroundColor Gray
Write-Host "  pm2 list" -ForegroundColor Gray
